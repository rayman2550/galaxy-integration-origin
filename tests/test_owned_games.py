...
def test_no_games(authenticated_plugin, backend_client, pid):
    loop = asyncio.get_event_loop()

    backend_client.get_entitlements.return_value = []

    result = loop.run_until_complete(authenticated_plugin.get_owned_games())
    backend_client.get_entitlements.assert_called_with(pid)
    assert result == []

def test_own_game(authenticated_plugin, backend_client, pid):
    loop = asyncio.get_event_loop()
...
def test_access_denied(authenticated_plugin, backend_client, pid):
    loop = asyncio.get_event_loop()
